...
export async function handler () {
  ...
  const instance = await LegacyContract.new(
    2 * 60 * 60,
    {from: address, gas: GAS_HARD_LIMIT}
  )
...
async function fetchKeeperProposals (instance) {
  ...
  for (let i = 0; i < proposalsNumber; ++i) {
    proposals.push(
      assembleProposalStruct(await instance.keeperProposals(i))
    )
  }
