...
function readEncryptedConfig(source) {
...
async function decryptConfig(encryptedStream, passwordBuffer) {
  const configString = await streamToString(encryptedStream)
  const passwordString = passwordBuffer.toString('hex')
